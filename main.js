    ...
    useEffect(() => {
        ...
        const handlePopState = (event) => {
            ...
            if (hasUnsavedChangesRef.current) {
                ...
                if (!leave) {
                    // 使用者選擇「留下來」 (取消)
                    // 這裡是最關鍵的一步：因為使用者按了返回，歷史紀錄已經少了一格
                    // 我們必須立刻把它「補回去」，這樣下次按返回時，才能再次觸發這個檢查
